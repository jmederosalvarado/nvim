...
local namespaces = setmetatable({}, {
	__index = function(t, key)
		local value = vim.api.nvim_create_namespace("vim_lsp_codelens:" .. key)
		rawset(t, key, value)
		return value
	end,
})
...
local function execute_lens(lens, bufnr, client_id)
	...
	vim.api.nvim_buf_clear_namespace(bufnr, namespaces[client_id], line, line + 1)
...
function M.clear(client_id, bufnr)
	...
	for _, iter_bufnr in pairs(buffers) do
		...
		for _, iter_client_id in pairs(client_ids) do
			...
			vim.api.nvim_buf_clear_namespace(iter_bufnr, ns, 0, -1)
		end
	end
end
...
function M.display(lenses, bufnr, client_id)
	...
	for i = 0, num_lines do
		...
		vim.api.nvim_buf_clear_namespace(bufnr, ns, i, i + 1)
		local chunks = {}
		...
		if #chunks > 0 then
			vim.api.nvim_buf_set_extmark(bufnr, ns, i, 0, {
				virt_lines = { { { string.rep(" ", vim.fn.indent(i)), "LspCodeLens" }, unpack(chunks) } },
	...
	if not lenses_by_client then
		...
		vim.api.nvim_buf_attach(bufnr, false, {
			...
			on_lines = function(_, b, _, first_lnum, last_lnum)
				vim.api.nvim_buf_clear_namespace(b, ns, first_lnum, last_lnum)
			end,
		})
	end
		...
		if lens.command then
			...
			client.request("codeLens/resolve", lens, function(_, result)
				if result and result.command then
					...
					vim.api.nvim_buf_set_extmark(bufnr, ns, lens.range.start.line, 0, {
						virt_lines = {
							{
								{ string.rep(" ", vim.fn.indent(lens.range.start.line)), "LspCodeLens" },
...
function M.on_codelens(err, result, ctx, _)
	if err then
		...
		local _ = vim.lsp.log.error() and vim.lsp.log.error("codelens", err)
		return
	end
...
function M.refresh(bufnr)
	...
	local bufnr = resolve_bufnr(bufnr)
